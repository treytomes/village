...
class TiledMap(TiledElement):
	...
	def reload_images(self):
		...
		for ts in self.tilesets:
			...
			# self.filename is the map path.  I need to combine with the tileset path.
			#TT: Calculate the image path relative to the TileSet it belongs to.  This may break embedded tilesets...
			#path = os.path.join(os.path.dirname(self.filename), ts.source)
			path = os.path.abspath(os.path.join(ts.path, ts.source))
	...
	def parse_xml(self, node):
		...
		if source:
			if source[-4:].lower() == ".tsx":
				...
				#TT: I added the self.path property so I can calculate paths relative to the TileSet asset.
