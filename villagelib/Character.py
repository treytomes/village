...
class Character:
    ...
    def __slide_position(self, map_manager, delta_x, delta_y):
        """
        All of this complication allows the character to slide around corners
        that would otherwise be hard to navigate using single-pixel movements.
        """
        tile_left, tile_top, tile_right, tile_bottom = \
            self.rect.left // map_manager.tile_width, self.rect.top // map_manager.tile_height, \
            (self.rect.right - 1) // map_manager.tile_width, (self.rect.bottom - 1) // map_manager.tile_height
        ...
        if delta_y < 0:  # self.facing_direction == NORTH:
            ...
            elif map_manager.collision_data[tile_top][tile_left] and not map_manager.collision_data[tile_top][
                tile_right]:
                self.rect.y -= delta_y
                self.rect.x += slide_speed
            ...
            if map_manager.collision_data[tile_bottom][tile_right] and not map_manager.collision_data[tile_bottom][
                tile_left]:
                self.rect.y -= delta_y
                self.rect.x -= slide_speed
            elif map_manager.collision_data[tile_bottom][tile_left] and not map_manager.collision_data[tile_bottom][
                tile_right]:
                self.rect.y -= delta_y
                self.rect.x += slide_speed
            elif map_manager.collision_data[tile_bottom][tile_left] and map_manager.collision_data[tile_bottom][
                tile_right]:
                self.rect.y -= delta_y
        elif delta_x > 0:  # self.facing_direction == EAST:
            if map_manager.collision_data[tile_bottom][tile_right] and not map_manager.collision_data[tile_top][
                tile_right]:
                self.rect.x -= delta_x
                self.rect.y -= slide_speed
            elif map_manager.collision_data[tile_top][tile_right] and not map_manager.collision_data[tile_bottom][
                tile_right]:
                self.rect.x -= delta_x
                self.rect.y += slide_speed
            elif map_manager.collision_data[tile_top][tile_right] and map_manager.collision_data[tile_bottom][
                tile_right]:
                self.rect.x -= delta_x
        elif delta_x < 0:  # self.facing_direction == WEST:
            if map_manager.collision_data[tile_bottom][tile_left] and not map_manager.collision_data[tile_top][
                tile_left]:
                self.rect.x -= delta_x
                self.rect.y -= slide_speed
            elif map_manager.collision_data[tile_top][tile_left] and not map_manager.collision_data[tile_bottom][
                tile_left]:
                self.rect.x -= delta_x
                self.rect.y += slide_speed
    ...
    def blit(self, screen, map_manager, player_rect):
        ...
        tile_left, tile_right, tile_bottom = self.rect.left // map_manager.tile_width, (
                self.rect.right - 1) // map_manager.tile_width, (self.rect.bottom - 1) // map_manager.tile_height
        if map_manager.is_tile_tall(tile_left, tile_bottom) and map_manager.is_tile_tall(tile_right, tile_bottom):
            self.sprite.tileset.blit_top(screen, self.sprite.animations.tile_index,
                                         SCREEN_CENTER_X + self.rect.x - player_rect.x,
                                         SCREEN_CENTER_Y + self.rect.y - player_rect.y)
        else:
            self.sprite.tileset.blit(screen, self.sprite.animations.tile_index,
                                     SCREEN_CENTER_X + self.rect.x - player_rect.x,
                                     SCREEN_CENTER_Y + self.rect.y - player_rect.y)
